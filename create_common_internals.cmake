...
#     PKG_INCLUDE_DIRS
#     PKG_LIBRARIES
#     PKG_QT_VERSION_MAJOR
#     PKG_QT_VERSION_MINOR
function( _create_common_internals_find_pkg )
  ...
  # Specific case when we use Qt through conan, AUTOGEN is not working unless Qt version is set
  if( NOT "${QT_VERSION_MAJOR}" STREQUAL "" )
    set( PKG_QT_VERSION_MAJOR ${QT_VERSION_MAJOR} PARENT_SCOPE )
    set( PKG_QT_VERSION_MINOR ${QT_VERSION_MINOR} PARENT_SCOPE )
  endif()
...
macro( _create_common_internals_handle_templates )
  ...
  if( PARSED_TEMPLATES )
    ...
    foreach( index RANGE ${nb_src_templates} )
      ...
      # Provide a warning info
      set( TEMPLATE_EDIT_WARNING "WARNING: File is generated by CMake from ${template} template file. DO NOT UPDATE MANUALLY. DO NOT COMMIT." )
...
macro( _create_common_internals_handle_gettext_translations )
  ...
  if( arg_GETTEXT_TRANSLATIONS )
    ...
    # Add the translations to the sources files list
    list( APPEND PARSED_SOURCES ${gettext_po_files} )
...
macro( _create_common_internals_handle_qt_translations )
  ...
  if( arg_QT_TRANSLATIONS )
    ...
    find_package(QT NAMES Qt6 Qt5 ${CREATE_LIBRARY_QUIET} COMPONENTS Core LinguistTools REQUIRED)
    find_package(Qt${QT_VERSION_MAJOR} ${CREATE_LIBRARY_QUIET} COMPONENTS Core LinguistTools REQUIRED)
    ...
    foreach( ts IN LISTS arg_QT_TRANSLATIONS )
      if( ts MATCHES ".ts$" )
        ...
        if( EXISTS ${CMAKE_CURRENT_LIST_DIR}/${ts} )
            list( APPEND PARSED_SOURCES ${ts} )
        endif()
      elseif( ts MATCHES ".qrc$" )
        list( APPEND ts_qrc ${ts} )
        message( STATUS "    ${ColorLib}${arg_NAME}${ColorReset} has ${ColorLib}Qt${QT_VERSION_MAJOR}${ColorReset} translations resources file ${ColorSrc}${ts}${ColorReset}" )
      else()
        list( APPEND ts_src ${CMAKE_CURRENT_LIST_DIR}/${ts} )
    ...
    if( QT_VERSION_MAJOR EQUAL 5 )
      qt5_create_translation( qm_files ${ts_src} ${ts_files} OPTIONS "-no-obsolete" )
    elseif( QT_VERSION_MAJOR EQUAL 6 )
      qt6_create_translation( qm_files ${ts_src} ${ts_files} OPTIONS "-no-obsolete" )
    else()
      message( SEND_ERROR "Qt translations via CMake only work with Qt5 or Qt6" )
    endif()

    # Add the translations to the sources files list
    list( APPEND PARSED_SOURCES ${qm_files} )

    # If a resources file is set, we create it with the list of qm files inside
    if( ts_qrc )
      get_filename_component( ts_qrc_filename "${ts_qrc}" NAME_WLE )
      unset( ts_qrc_content )
      set( ts_qrc_content "${ts_qrc_content}<!-- WARNING: File is generated by CMake from translation files. DO NOT UPDATE MANUALLY. DO NOT COMMIT. -->\n")
      set( ts_qrc_content "${ts_qrc_content}<RCC>\n  <qresource prefix=\"/${ts_qrc_filename}\">\n")
      foreach( qm IN LISTS qm_files )
        get_filename_component( qm_filename "${qm}" NAME_WLE )
        set( ts_qrc_content "${ts_qrc_content}    <file alias=\"${qm_filename}\">${qm}</file>\n")
      endforeach()
      set( ts_qrc_content "${ts_qrc_content}  </qresource>\n</RCC>")
      file( WRITE ${ts_qrc} "${ts_qrc_content}" )
    endif()
...
macro( _create_common_internals_handle_packages_dependencies )
  ...
  if( PARSED_PACKAGE_DEPENDENCIES )
    ...
    elseif( libraries_pub MATCHES "qt::qt" OR libraries_priv MATCHES "qt::qt" )
      # Specific case when we use Qt through conan, AUTOGEN is not working unless Qt version is set
      message( STATUS "    ${ColorLib}${arg_NAME}${ColorReset} uses Qt so it has properties ${ColorFlags}AUTOMOC AUTOUIC AUTORCC${ColorReset}" )
      set( QT_VERSION_MAJOR ${PKG_QT_VERSION_MAJOR} PARENT_SCOPE )
      set( QT_VERSION_MINOR ${PKG_QT_VERSION_MINOR} PARENT_SCOPE )
      set_target_properties( ${arg_NAME} PROPERTIES AUTOMOC ON AUTOUIC ON AUTORCC ON )
    endif()
  endif()
